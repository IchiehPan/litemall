...
@Service
public class LitemallMultimallAdService {
    @Resource
    private LitemallAdMapper adMapper;

    public List<LitemallAd> queryIndex(Integer mallId) {
        LitemallAdExample example = new LitemallAdExample();
        LitemallAdExample.Criteria criteria = example.or().andPositionEqualTo((byte) 1).andDeletedEqualTo(false).andEnabledEqualTo(true);
    ...
    public List<LitemallAd> querySelective(String name, String content, Integer mallId, Integer page, Integer limit, String sort, String order) {
        LitemallAdExample example = new LitemallAdExample();
