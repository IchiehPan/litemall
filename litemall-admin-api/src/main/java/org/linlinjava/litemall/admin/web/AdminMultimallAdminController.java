...
import org.linlinjava.litemall.db.service.LitemallMultimallAdminService;
...
@RestController
@RequestMapping("/admin/multimall/admin")
@Validated
public class AdminMultimallAdminController {
    private final Log logger = LogFactory.getLog(AdminMultimallAdminController.class);

    @Autowired
    private LitemallMultimallAdminService adminService;
    @Autowired
    private LitemallMultimallService multimallService;
    @Autowired
    private LogHelper logHelper;

    @RequiresPermissions("admin:multimalladmin:list")
    @RequiresPermissionsDesc(menu = {"系统管理（多商户）", "管理员管理"}, button = "查询")
    @GetMapping("/list")
    ...
    private Object validate(LitemallAdmin admin) {
        ...
        Integer mallId = admin.getMallId();
        if (mallId == null || multimallService.findById(mallId) == null) {
            return ResponseUtil.fail(ADMIN_INVALID_MALL, "管理员所属商铺不存在");
        }

        return null;
    }

    @RequiresPermissions("admin:multimalladmin:create")
    @RequiresPermissionsDesc(menu = {"系统管理（多商户）", "管理员管理"}, button = "添加")
    @PostMapping("/create")
    ...
    @RequiresPermissions("admin:multimalladmin:read")
    @RequiresPermissionsDesc(menu = {"系统管理（多商户）", "管理员管理"}, button = "详情")
    @GetMapping("/read")
    ...
    @RequiresPermissions("admin:multimalladmin:update")
    @RequiresPermissionsDesc(menu = {"系统管理（多商户）", "管理员管理"}, button = "编辑")
    @PostMapping("/update")
    ...
    @RequiresPermissions("admin:multimalladmin:delete")
    @RequiresPermissionsDesc(menu = {"系统管理（多商户）", "管理员管理"}, button = "删除")
    @PostMapping("/delete")
