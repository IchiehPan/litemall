...
import org.linlinjava.litemall.db.service.LitemallMultimallCouponService;
...
@RestController
@RequestMapping("/admin/multimall/coupon")
@Validated
public class AdminMultimallCouponController {
    private final Log logger = LogFactory.getLog(AdminMultimallCouponController.class);
    ...
    @RequiresPermissions("admin:multimallcoupon:list")
    @RequiresPermissionsDesc(menu = {"推广管理（多商户）", "优惠券管理"}, button = "查询")
    @GetMapping("/list")
    public Object list(String name, Short type, Short status,
        ...
        List<LitemallCoupon> couponList = couponService.querySelective(name, type, status, mallId, page, limit, sort, order);
        return ResponseUtil.okList(couponList);
    }

    @RequiresPermissions("admin:multimallcoupon:listuser")
    @RequiresPermissionsDesc(menu = {"推广管理（多商户）", "优惠券管理"}, button = "查询用户")
    @GetMapping("/listuser")
    ...
    private Object validate(LitemallCoupon coupon) {
        ...
        Integer mallId = coupon.getMallId();
    ...
    @RequiresPermissions("admin:multimallcoupon:create")
    @RequiresPermissionsDesc(menu = {"推广管理（多商户）", "优惠券管理"}, button = "添加")
    @PostMapping("/create")
    ...
    @RequiresPermissions("admin:multimallcoupon:read")
    @RequiresPermissionsDesc(menu = {"推广管理（多商户）", "优惠券管理"}, button = "详情")
    @GetMapping("/read")
    ...
    @RequiresPermissions("admin:multimallcoupon:update")
    @RequiresPermissionsDesc(menu = {"推广管理（多商户）", "优惠券管理"}, button = "编辑")
    @PostMapping("/update")
    ...
    @RequiresPermissions("admin:multimallcoupon:delete")
    @RequiresPermissionsDesc(menu = {"推广管理（多商户）", "优惠券管理"}, button = "删除")
    @PostMapping("/delete")
