 ...
 */
@RestController
@RequestMapping("/wx/multimall/catalog")
@Validated
public class WxMultimallCatalogController {
    private final Log logger = LogFactory.getLog(WxMultimallCatalogController.class);

    @Autowired
    private LitemallMultimallCategoryService categoryService;

    @GetMapping("/getfirstcategory")
    public Object getFirstCategory(@NotNull Integer mallId) {
        // 所有一级分类目录
    ...
    public Object getSecondCategory(@NotNull Integer id, @NotNull Integer mallId) {
        // 所有二级分类目录
        List<LitemallCategory> currentSubCategory = categoryService.queryByPid(id, mallId);
     ...
     */
    @GetMapping("index")
    public Object index(Integer id, @NotNull Integer mallId) {
        ...
        if (null != currentCategory) {
            currentSubCategory = categoryService.queryByPid(currentCategory.getId(), mallId);
        }
     ...
     */
    @GetMapping("all")
    public Object queryAll(@NotNull Integer mallId) {
        //优先从缓存中读取
        ...
        for (LitemallCategory category : l1CatList) {
            sub = categoryService.queryByPid(category.getId(), mallId);
     ...
     */
    @GetMapping("current")
    public Object current(@NotNull Integer id, @NotNull Integer mallId) {
        // 当前分类
        LitemallCategory currentCategory = categoryService.findById(id);
        if (currentCategory == null) {
            return ResponseUtil.badArgumentValue();
        }
        List<LitemallCategory> currentSubCategory = categoryService.queryByPid(currentCategory.getId(), mallId);
